...
async def approve_web_access(callback: CallbackQuery, session: AsyncSession, bot: Bot) -> None:
    """Approve browser login request and generate permanent JWT."""
    ...
    # Note: create_access_token generated a token valid for 7 days based on current config,
    # but the user requested "навсегда". We can make it longer if needed, but 7 days is
    # the default in dependencies.py. We will just use the default.
