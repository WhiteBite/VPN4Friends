...
async def test_speed(proxy_url: str, test_size_mb: int = 10) -> dict:
    ...
    print("⏳ Testing speed through proxy...")
    ...
    if results:
        ...
        return {"average_mbps": avg_speed, "tests": results}
...
async def test_with_xray(vless_url: str) -> dict:
    ...
    print("🚀 Starting xray-core on SOCKS5 localhost:10808...")

    # Start xray
    try:
        proc = subprocess.Popen(
            ["xray", "run", "-c", config_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            async with session.get(
                test_url, proxy=proxy, timeout=aiohttp.ClientTimeout(total=60)
            ) as response:
                downloaded = 0
                async for chunk in response.content.iter_chunked(8192):
                    downloaded += len(chunk)
